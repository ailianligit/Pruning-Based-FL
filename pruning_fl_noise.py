...
parser.add_argument('--tradeoff', default=0.5, type=float)
...
parser.add_argument('--outfile2', default='output2.csv', type=str)
...
pers = [1, 1, 2, 2, 2, 5, 5, 5, 10000, 10000]
...
class Client:
    ...
    def __init__(self, id, device, train_data, net=models.MNISTNet,
        ...
        for inputs, labels in train_data:
            new_inputs = inputs.clone().to(device=device)
            if i % pers[id] == 0:
                new_labels = torch.randint_like(labels, low=0, high=10, device=device)
            else:
                new_labels = labels.clone().to(device=device)
            correct += sum(labels == new_labels)
            total += len(labels)
            i += 1
            new_train_data.append(tuple([new_inputs, new_labels]))
...
with open(args.outfile2,'w') as f2:
...
for server_round in tqdm(range(args.rounds)):
    ...
    if args.need_readjust:
        if args.rate_decay_method == 'cosine':
            readjust_ratio = global_model._decay(server_round, alpha=args.readjustment_ratio, t_end=args.rate_decay_end)
        else:
            readjust_ratio = args.readjustment_ratio
        readjust = server_round % args.rounds_between_readjustments == 0 and readjust_ratio > 0.
        if readjust:
            dprint('readjusting', readjust_ratio)
    ...
    round_tradeoff = 1 - (1 - args.tradeoff) / (server_round + 1)
    ...
    for client_id in client_indices:
        ...
        important_score[i] = round_tradeoff * important_score[i] + (1 - round_tradeoff) * psi
    ...
    # if readjust and round_sparsity:
    #     prune_sparsity = round_sparsity + (1 - round_sparsity) * readjust_ratio
    #     global_model.layer_prune(sparsity=prune_sparsity, sparsity_distribution=args.sparsity_distribution)
    #     global_model.layer_grow(sparsity=round_sparsity, sparsity_distribution=args.sparsity_distribution, update=agg_update)
    # elif round_sparsity:
    ...
    if readjust and round_sparsity:
        prune_sparsity = round_sparsity + (1 - round_sparsity) * readjust_ratio
        global_model.layer_prune(sparsity=prune_sparsity, sparsity_distribution=args.sparsity_distribution)
        global_model.layer_grow(sparsity=round_sparsity, sparsity_distribution=args.sparsity_distribution)
    elif round_sparsity:
        global_model.layer_prune(sparsity=round_sparsity, sparsity_distribution=args.sparsity_distribution)
    ...
    with open(args.outfile2,'a+') as f2:
        csv_write = csv.writer(f2)
    ...
    if server_round % args.eval_every == 0 and args.eval:
        with open(args.outfile,'a+') as f:
            ...
            data_row = [server_round,
                        ...
                        round_tradeoff,
                        args.need_readjust,
                        args.rounds_between_readjustments,
                        readjust,
                        readjust_ratio,
                        args.seed
                        ]
