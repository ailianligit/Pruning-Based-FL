...
class PrunableNet(nn.Module):
    ...
    def init_param_sizes(self):
        ...
        self.param_size = 0
        self.flops = 0
        for lname, layer in self.named_children():
            for name, param in layer.named_parameters():
    ...
    def calculate_flops(self, data_size, h_in, w_in, sparsity):
        flops = 0.0
        for lname, layer in self.named_children():
            if isinstance(layer, nn.modules.conv._ConvNd):
                ...
                flops += 2 * data_size * neur_in * kernel_size[0] * kernel_size[1] * h_out * w_out * neur_out * (1 + 2 * (1-sparsity))
                h_in = h_out
                w_in = w_out
            elif isinstance(layer, nn.Linear):
                neur_out = layer.out_features
                neur_in = layer.in_features
                flops += 2 * data_size * neur_out * neur_in * (1 + 2 * (1-sparsity))
        return flops
        ...
        with torch.no_grad():
            ...
            for name, layer in self.named_children():
                ...
                for pname, param in layer.named_parameters():
                    ...
                    # else:
                    #     _, prune_indices = torch.topk(torch.abs(param.grad.flatten()),
                    #                                   n_prune, largest=False)
        ...
        with torch.no_grad():
            ...
            for name, layer in self.named_children():
                ...
                for pname, param in layer.named_parameters():
                    ...
                    self.mask = torch.zeros_like(param.data, dtype=torch.bool, device=self.device)
                    for bname, buf in layer.named_buffers():
                        if bname == pname + '_mask':
                            self.mask.copy_(buf)
                    self.mask = ~self.mask

                    # update_name = name + '.' + pname
                    self.update = torch.randn(size=param.data.size(), dtype=torch.float, device=self.device)
                    # Determine largest gradient indices
                    _, grow_indices = torch.topk(torch.abs(self.update.flatten() * self.mask.flatten()),
                                                  n_grow, largest=True)
                    # _, grow_indices = torch.topk(torch.abs(update[update_name].flatten() * self.mask.flatten()),
                    #                              n_grow, largest=True)

                    # _, grow_indices = torch.topk(torch.abs(param.grad.flatten() * self.mask.flatten()),
                    #                               n_grow, largest=True)
    ...
    def reset_mask(self, global_state):
        with torch.no_grad():
            for name, param in global_state.items():
                ...
                mask_name = name + '_mask'
                if needs_mask(name):
                    self.new_state[mask_name] = global_state[mask_name].clone().to(self.device)
                self.new_state[name] = param.clone().to(self.device)
            self.load_state_dict(self.new_state)
    ...
    def apply_mask(self):
        with torch.no_grad():
            for name, param in self.state_dict().items():
                ...
                mask_name = name + '_mask'
                if needs_mask(name):
                    mask_to_apply = self.state_dict()[mask_name].to(self.device, dtype=torch.bool)
                    self.new_state[name] = param.clone().to(self.device)
                    self.new_state[name][~mask_to_apply] = 0
                    # self.gpu_param = param[mask_to_apply].clone().to(self.device)
                    # self.new_state[name] = torch.zeros_like(param, dtype=torch.float, device=self.device)
                    # self.new_state[name][mask_to_apply] = self.gpu_param
                    self.new_state[mask_name] = self.state_dict()[mask_name].clone().to(self.device)
                else:
                    self.new_state[name] = param.clone().to(self.device)
            self.load_state_dict(self.new_state)
        ...
        with torch.no_grad():
            ...
            for name, param in param_source.items():
                ...
                if needs_mask(name) and mask_name in apply_mask_source:
                    ...
                    mask_to_apply = apply_mask_source[mask_name].to(device='cpu', dtype=torch.bool)
                    ...
                    if mask_name in local_state:
                        new_state[mask_name] = torch.ones_like(local_state[mask_name])
