...
def get_global_dataset(dataset='mnist', mode='dirichlet', path=None, clients=400,
    ...
    if dataset == 'mnist':
        ...
        test = torchvision.datasets.MNIST(path, train=False, download=False, transform=xfrm)
    elif dataset == 'cifar10':
        xfrm = torchvision.transforms.Compose([
            torchvision.transforms.ToTensor(),
            torchvision.transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))
        ])
        test = torchvision.datasets.CIFAR10(path, train=False, download=False, transform=xfrm)
    elif dataset == 'cifar100':
        xfrm = torchvision.transforms.Compose([
            torchvision.transforms.ToTensor(),
            torchvision.transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))
        ])
        test = torchvision.datasets.CIFAR100(path, train=False, download=False, transform=xfrm)
    
    test_idx = torch.arange(len(test))

    # shuffle
    test_sampler = rng.choice(test_idx, size=test_idx.shape, replace=False)
...
def distribute_clients_dirichlet(train, clients=400, batch_size=32, beta=0.1, rng=None):
...
def distribute_iid(train, clients=400, samples_per_client=40, batch_size=32, rng=None):
...
def get_mnist_or_cifar10(dataset='mnist', mode='dirichlet', path=None, clients=400,
    ...
    if mode in ('dirichlet', 'iid'):
        if dataset == 'mnist':
            ...
            train = torchvision.datasets.MNIST(path, train=True, download=False, transform=xfrm)
            # test = torchvision.datasets.MNIST(path, train=False, download=False, transform=xfrm)
            ...
            train = torchvision.datasets.CIFAR10(path, train=True, download=False, transform=xfrm)
            # test = torchvision.datasets.CIFAR10(path, train=False, download=False, transform=xfrm)
        elif dataset == 'cifar100':
            xfrm = torchvision.transforms.Compose([
                torchvision.transforms.ToTensor(),
            ...
            train = torchvision.datasets.CIFAR100(path, train=True, download=False, transform=xfrm)
            # test = torchvision.datasets.CIFAR100(path, train=False, download=False, transform=xfrm)

        if mode == 'dirichlet':
            # train_idx, test_idx = distribute_clients_dirichlet(train, test, clients=clients, beta=beta, rng=rng)
            train_idx = distribute_clients_dirichlet(train, clients=clients, beta=beta, rng=rng)
        elif mode == 'iid':
            # train_idx, test_idx = distribute_iid(train, test, clients=clients, samples_per_client=samples, rng=rng)
            train_idx = distribute_iid(train, clients=clients, samples_per_client=samples, rng=rng)
    ...
    for i in range(clients):
        ...
        # test_loader = torch.utils.data.DataLoader(test, batch_size=batch_size, sampler=test_sampler)
...
def get_dataset(dataset, devices=None, **kwargs):
    ...
    **kwargs
    ...
    for i, (uid, train_loader) in enumerate(loaders.items()):
        device = devices[i % len(devices)]
        train_data = [(x.to(device), y.to(device)) for x, y in train_loader]
        # test_data = [(x.to(device), y.to(device)) for x, y in test_loader]

        # new_loaders[uid] = (device, train_data, test_data)
        new_loaders[uid] = (device, train_data)
